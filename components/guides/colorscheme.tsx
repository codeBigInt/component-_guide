  ...
  const renderColors = (colorObj: any, prefix: string = ''): any => {
    return Object.entries(colorObj).map(([key, value]) => {
      if (typeof value === 'string') {
        const className = key === 'DEFAULT' ? `${prefix.slice(0, -1)}` : `${prefix}${key}`;
        const isDark = key.match(/(600|700|800|900|C600|B900| )/);
        return (
          <div
            key={className}
            className={`flex items-center space-x-2 cursor-pointer p-2 rounded-md ${isDark ? 'text-white' : 'text-black'}`}
            onClick={() => copyToClipboard(`bg-${className}`)}
          >
            <div className="w-full flex items-center justify-center rounded-md p-5" style={{ backgroundColor: value }}>
              {`bg-${className}`}
            </div>
      ...
      const nestedPrefix = key.match(/\d/) ? `${prefix}${key}-` : `${prefix}${key}-`;
